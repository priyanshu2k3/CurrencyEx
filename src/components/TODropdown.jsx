...
function TODropdown() {
        const {TOcurrency, setTOcurrency} =useContext(statevar);

    async function  changeCurrency(e){
        await setTOcurrency(e);
    ...
    <label>TO
        ...
        <select onChange={(event) => changeCurrency(event.target.value)} value={TOcurrency} className="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
